...
const MyClasses = () => {
  ...
  useEffect(() => {
    const fetchMyClasses = async () => {
      try {
        const { data } = await API.get('/learner/my-classes');
  ...
  if (loading) return <LearnerLayout><div style={{padding:'20px', color: 'white'}}>Loading your classes...</div></LearnerLayout>;
      ...
      <div style={{ maxWidth: '1200px', margin: '0 auto', padding: '40px 20px' }}>
        <div style={{ marginBottom: '60px' }}>
          <h1 style={{ fontSize: '2.5rem', textShadow: '0 0 20px rgba(59,130,246,0.1)' }}>
            MY <span style={{ color: '#3b82f6' }}>CLASSES</span>
          </h1>
          <p style={{ fontSize: '1.1rem', color: 'var(--text-muted)' }}>
            Your enrolled live workshops and interactive sessions.
          </p>
        </div>
        ...
        {classes.length > 0 ? (
                ...
                <div style={{ padding: '20px', flex: 1, display: 'flex', flexDirection: 'column' }}>
                  ...
                  <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginTop: 'auto' }}>
                    <span style={{ fontSize: '0.65rem', color: 'var(--primary)', fontWeight: '700' }}>
                      📅 {cls.rounds?.[0]?.start ? new Date(cls.rounds[0].start).toLocaleDateString() : 'Active'}
                    </span>
                    <span style={{ fontSize: '0.7rem', color: '#3b82f6', fontWeight: '900' }}>LEARN →</span>
                  </div>
                </div>
              </div>
            ))}
          ...
          <div style={{ textAlign: 'center', padding: '80px 0', border: '1px dashed var(--border)', borderRadius: '16px' }}>
            <p style={{ color: 'var(--text-muted)', marginBottom: '20px' }}>You haven't enrolled in any classes yet.</p>
            <button className="btn btn-primary" style={{ background: '#3b82f6' }} onClick={() => navigate('/learner/class-catalog')}>Browse Class Catalog</button>
          </div>
        )}
      </div>
    </LearnerLayout>
